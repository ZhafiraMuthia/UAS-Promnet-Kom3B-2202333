...
func CreateBook(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("INSERT INTO peminjamanBuku_ZhafiraMuth(judul_buku, jumlah, nama_peminjam, alamat_peminjam, noHp_peminjam, tanggal_pinjam, tanggal_pengembalian, lama_pinjam) VALUES(?,?,?,?,?,?,?,?)")
	if err != nil {
		panic(err.Error())
	}
...
func GetBooks(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id, judul_buku, jumlah, nama_peminjam, alamat_peminjam, noHp_peminjam, tanggal_pinjam, tanggal_pengembalian, lama_pinjam FROM peminjamanBuku_ZhafiraMuth")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func GetBookByID(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id, judul_buku, jumlah, nama_peminjam, alamat_peminjam, noHp_peminjam, tanggal_pinjam, tanggal_pengembalian, lama_pinjam FROM peminjamanBuku_ZhafiraMuth WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateBook(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("UPDATE peminjamanBuku_ZhafiraMuth SET judul_buku=?, jumlah=?, nama_peminjam=?, alamat_peminjam=?, noHp_peminjam=?, tanggal_pinjam=?, tanggal_pengembalian=?, lama_pinjam=? WHERE id=?")
	if err != nil {
		panic(err.Error())
	}
...
func DeleteBook(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("DELETE FROM peminjamanBuku_ZhafiraMuth WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func InitDB() {
	db, err = sql.Open("mysql", "root:@tcp(127.0.0.1:3306)/db_2202333_ZhafiraMuth_uas_pilkomB")
	if err != nil {
		panic(err.Error())
	}
}
